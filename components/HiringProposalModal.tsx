...
interface HiringProposalModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const HiringProposalModal: React.FC<HiringProposalModalProps> = ({ isOpen, onClose, data, applicantProfile, interviewText }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        pdf.save(`채용품의서_${applicantProfile.applicantName || '지원자'}_${today}.pdf`);
    };


    const formatSalary = (value?: number) => value ? `${value.toLocaleString()}만원` : '-';
    ...
    return (
        <div 
          className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 hiring-proposal-printable-area"
          onClick={onClose}
        >
            <div 
            ...
            >
                <main className="p-8 overflow-y-auto" id="hiring-proposal-content">
                    <div ref={proposalContentRef} className="text-sm" style={{ color: '#000' }}>
                        <div ref={page1Ref}>
                            <h1 className="text-3xl font-bold text-center mb-4">채용품의서</h1>
                            <p className="text-center text-slate-500 mb-[50px]">인사_002_10</p>
                            ...
                            <table className="w-full border-collapse border-2 border-slate-400 mb-6">
                                <tbody>
                                    <tr className="border-b border-slate-300">
                                        ...
                                        <TdContent isEditable>{applicantProfile.applicantName || '-'}</TdContent>
                            ...
                            <table className="w-full border-collapse border-2 border-slate-400 mb-6">
                                <tbody>
                                    ...
                                    <tr className="border-b border-slate-300">
                                        ...
                                        <TdContent isEditable isSalary>
                                            {data.companyStandard && typeof data.companyStandard !== 'string' && data.companyStandard.middle 
                                                ? formatSalary(data.companyStandard.middle) 
                                                : '-'}
                                        </TdContent>
                ...
                <footer className="hiring-proposal-no-print p-4 border-t border-slate-200 mt-auto flex justify-center gap-3 bg-white/50 rounded-b-2xl">
